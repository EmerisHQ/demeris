...
// TODO: This is a copy of the old v0.1.52 @starport/vuex which does not play well with vite. Most of the functionality is not needed. Should be replaced once liquidity modules are updated

const apiNode =
  (import.meta.env.VITE_API_COSMOS && (import.meta.env.VITE_API_COSMOS as string).replace('0.0.0.0', 'localhost')) ||
  'http://localhost:1317';
const rpcNode =
  (import.meta.env.VITE_API_TENDERMINT &&
    (import.meta.env.VITE_API_TENDERMINT as string).replace('0.0.0.0', 'localhost')) ||
  'http://localhost:26657';
const wsNode =
  (import.meta.env.VITE_WS_TENDERMINT &&
    (import.meta.env.VITE_WS_TENDERMINT as string).replace('0.0.0.0', 'localhost')) ||
  'ws://localhost:26657/websocket';
const addrPrefix = import.meta.env.VITE_ADDRESS_PREFIX || 'cosmos';
  ...
  actions: {
    async init(
      ...
      try {
        ...
        throw new EmerisError('Env:Config', 'Could not configure environment');
      }
    },
        ...
        if (!state.client) {
          ...
          if (reconnectWS && config.wsNode) {
            try {
              ...
              throw new EmerisError('Env:Client:Websocket', 'Could not switch to websocket node:' + config.wsNode);
            }
          }
