  ...
  async [ActionTypes.SIGN_IN]({ commit, dispatch, rootGetters }) {
    try {
      ...
      if (!isCypress) {
        ...
        signer = await Secp256k1HdWallet.fromMnemonic(import.meta.env.EMERIS_MNEMONIC, {
      ...
      for (const chain of toQuery) {
        if (!isCypress) {
          ...
          const signer = await Secp256k1HdWallet.fromMnemonic(import.meta.env.EMERIS_MNEMONIC, {
