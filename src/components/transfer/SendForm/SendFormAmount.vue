  ...
  <div :class="{ 'send-form-amount--insufficient-funds': !hasSufficientFunds }">
    <fieldset class="form__field send-form-amount">
      <div class="send-form-amount__input">
        <input v-model="form.amount.amount" class="send-form-amount__input__control" min="0" placeholder="0" />
...
export default defineComponent({
  ...
  setup(_, { emit }) {
    ...
    const hasSufficientFunds = computed(() => {
      ...
      return state.currentAsset.chains[0].amount >= +form.amount.amount;
    });
    ...
    const setCurrentAsset = (asset: Record<string, unknown>) => {
      ...
      form.amount.denom = asset.denom as string;
    };
    ...
    watch(
      ...
      () => {
        if (state.isMaximumAmountChecked) {
          form.amount.amount = state.currentAsset.chains[0].amount;
        }
      },
    );
